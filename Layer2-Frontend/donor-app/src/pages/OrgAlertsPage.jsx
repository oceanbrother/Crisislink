      ...
      <main className="feed-content org-feed-content">
          ...
          <div className="org-page-heading-row">
            <div className="org-page-heading">
              ...
              <p className="org-page-subtitle">
                {t('dashboard.intelligence.subtitle', 'Preview upcoming demand spikes in your nearby service postcodes.')}
              </p>
        ...
        {loading ? (
          ...
          <section className="empty-state empty-state--rich" aria-live="polite">
            ...
            <p className="empty-state-subtitle">
              {t(
                ...
                'We could not load listings required for this alerts view. Please try again.',
              )}
            </p>
          </section>
        ) : !topAlert ? (
          <section className="empty-state empty-state--rich" aria-live="polite">
            ...
            <h2 className="empty-state-title">
              {t('dashboard.intelligence.noDataTitle', 'No alerts data yet')}
            </h2>
            <p className="empty-state-subtitle">
              {t('dashboard.intelligence.noDataHint', 'Demand alerts will appear here once forecasting data is available.')}
            </p>
          </section>
        ) : (
          <section className="org-demand-section" aria-labelledby="org-demand-intelligence">
            <div className="org-demand-heading">
              ...
              <h2 id="org-demand-intelligence">{t('dashboard.intelligence.title', 'Demand intelligence')}</h2>
              <p>{t('dashboard.intelligence.subtitle', 'Preview upcoming demand spikes in your nearby service postcodes.')}</p>
              {demandInsights.source === 'sample' ? (
                <p className="org-demand-data-note">
                  {t(
                    'dashboard.intelligence.sampleNote',
                    'Sample demand indicators are shown until the forecasting data feed is connected.',
                  )}
                </p>
              ) : null}
            </div>
            ...
            <article className="org-demand-primary-card">
              <div className="org-demand-primary-topline">
                ...
                <span>{t('dashboard.intelligence.primaryLabel', 'Demand spike alert')}</span>
              </div>

              <div
                className="org-demand-priority-row"
                aria-label={t('dashboard.intelligence.priorityCue', 'Alert priority and trigger')}
              >
              ...
              <h3>
                {t('dashboard.intelligence.primaryHeadline', {
                  ...
                  defaultValue: '{{suburb}} ({{postcode}}) may need extra support soon',
                })}
              </h3>
              ...
              <div className="org-demand-evidence">
                <div className="org-demand-evidence-row">
                  <span>{t('dashboard.intelligence.fields.predictedWindow', 'Predicted window')}</span>
                  <strong>{topAlert.predictedWindow || '-'}</strong>
                </div>
                <div className="org-demand-evidence-row">
                  <span>{t('dashboard.intelligence.fields.confidence', 'Forecast confidence')}</span>
                  <strong>{topAlert.confidence}%</strong>
                </div>
                <div className="org-demand-evidence-row">
                  <span>{t('dashboard.intelligence.fields.factors', 'Contributing factors')}</span>
              ...
              <div className="org-demand-metrics">
                <div className="org-demand-metric">
                  <span>{t('dashboard.intelligence.metrics.demandLift', 'Demand lift')}</span>
                  <strong>+{topAlert.demandLift}%</strong>
                </div>
                <div className="org-demand-metric">
                  <span>{t('dashboard.intelligence.metrics.activePortions', 'Active portions')}</span>
                  <strong>{topAlert.activePortions}</strong>
                </div>
                <div className="org-demand-metric">
                  <span>{t('dashboard.intelligence.metrics.households', 'Households at risk')}</span>
              ...
              >
                {t('dashboard.intelligence.respondAction', 'Respond to this need')}
              </button>
            </article>
          </section>
        )}
